...
## Review Fix Verification (2026-09-16)

Four review findings were reproduced by failing regression tests, then fixed:

- PDF export with an unknown port and no B0 now retains the error report instead of raising `KeyError`.
- CSV/PDF decision summaries preserve every recommendation role, including multiple roles selecting B0 or the same candidate scenario.
- CSV EU ETS savings use the same sign convention for amount and percentage; zero, missing values and `ZERO_BASELINE` retain their meaning.
- The web summary matches the exact `CURRENT_MODEL_COST_MIN` identifier and shows the missing-price reason. Supplying the price restores the recommendation.

Production edits are limited to `reports.py` and `static/app.js`. Calculation
formulas, factor and port data, raw result contracts, and main remain unchanged.
The patch adds six test methods (five report/API tests and one browser test).

Verification on the feature worktree:

- `python -m unittest discover -s tests -q`: 221 passed, including 15 browser tests.
- Separate desktop/mobile browser run: 15 passed, including CSV/PDF downloads and display precision.
...
- Independent scoped review found no actionable issues in the repair diff.
- The PDF decision-summary page was rendered and visually checked.

The root checkout's Python virtual environment was used with `PYTHONPATH=src`
to test this worktree. Separate verification artifacts are retained under the
ignored `output/review-fixes/` directory. Tracked browser snapshots rewritten
by full test discovery were restored; they are not part of the repair.

The test environment emits an existing Starlette/httpx deprecation warning.
Poppler emitted missing optional-font warnings while rendering, but the summary
page rendered legibly. No dependency changes were made for those warnings.
This verification does not imply a merge to main or completed real-voyage acceptance.

## Independent Cross-Validation (2026-09-16)

User-requested algorithm-independent checks are recorded in
`docs/validation/new-energy-cross-validation.md`, with machine-readable results
in `docs/validation/new-energy-cross-validation-results.json`.

The dedicated suite passed 18 test methods, covering 60 synthetic single-candidate
inputs, 10 seeded multi-candidate portfolios, 3 hand-calculated portfolio/order
examples, one existing built-in snapshot, and three hand cases across desktop
and mobile page/API/CSV/PDF surfaces. Exact mass-space vertex optima are compared
with HiGHS and production decisions; expected winners are not generated by the
production scenario builder. Seven intentionally corrupted output variants are
rejected. Full regression remains 221 Python tests and 29 Node tests passing.

No production calculation or data changes were needed. These checks test the
frozen mathematical contract, not factor provenance or real-voyage acceptance.
Missing-price endpoint optimization and numerical-solver precision limitations
are explicitly scoped in the validation report.

## Expanded Validation Gate (2026-09-16)

The second validation pass adds 259 single-candidate cases (319 total), 40
portfolios (50 seeded portfolios total, plus the original three hand orderings),
scaling/constraint relations, six hand surface cases at two widths, and invalid
API input tests. The former all-pass statement above describes the first pass only.

The expanded API test exposes three failing subcases: negative custom `cfCO2`,
`cfCH4`, and `cfN2O` are accepted as comparable and can contaminate recommendations.
The failing test is retained without skipping or expected-failure suppression.
No production calculation/data change was made in this testing-only pass.
At that point, merge readiness was blocked on addressing this input-validation
defect and rerunning the expanded suite. Details and reproduction are retained
in the validation report.

## Input Validation Repair (2026-09-16)

Custom combustion factors and active slip emission factors now reject negative
and nonfinite values with `INVALID_EMISSION_FACTOR`. Zero, existing `NA`
semantics, unused slip factors and negative WtT remain supported. No formulas,
fuel catalog or port data changed.

New regression tests first reproduced 30 invalid-factor failures. After repair,
the custom-factor suite passed 10 tests; full Python regression passed 225 tests;
the expanded independent suite passed all 27 methods (319 single-candidate
cases, 50 seeded portfolios, six hand surface cases); Node passed 29 tests.
Compilation, JavaScript syntax and whitespace checks passed. Independent
read-only review found no actionable issues in the two-file repair diff.

The input-validation blocker is resolved. Changes remain on the feature branch;
main integration and real-voyage business acceptance are separate.
