...
> 状态：已确认；Python 计算内核第一阶段及完整内置因子目录已实现
>
> 日期：2026-08-07
>
> 2026-09-07 修订（codex/c-layer-correctness-fixes）：补充达标可行区间、临界比例安全取值和精确切换判定；尚未合入 main。
>
...
- 目标残差、达标区间和方案切换允许使用从输入 Decimal 精确转换的有理数作内部决策；API 仍输出 Decimal 十进制字符串，不经过 float，不增加生产数值依赖。
- 生成的最低达标比例向可行侧向上取值，达标上界向可行侧向下取值；保留当前高精度上下文，不按页面显示位数取值。用户输入比例不作这种调整。不得把负余额用容差归零为达标。
- 两组分质量使用同一总质量计算，并精确保留比例及其补数，避免分别舍入组分质量改变临界点组成。循环小数总质量仍受高精度表示限制，能源守恒按上述跨实现数值精度验收。
- 余额从精确残差 `(target×D-N)×E_scoped/D` 计算，避免先将 GHGI 投影为有限 Decimal 后再相减丢失符号。极端临界点的 GHGI 显示可等于目标，但未舍入余额非零时仍按真实符号分类。
- `-0`在输出边界统一规范化为`0`。
...
| --- | --- |
| `h_b<=0` 且 `h_c<=0` | `[0,1]` |
| `h_b>0` 且 `h_c<=0` | `[h_b/(h_b-h_c),1]` |
| `h_b<=0` 且 `h_c>0` | `[0,-h_b/(h_c-h_b)]` |
| `h_b>0` 且 `h_c>0` | 空集 |

该区间与 `[0,xCap]` 相交后得到 `[xTargetLow,xTargetHigh]`。两个符号为零的退化情况由表中前两项覆盖，不作除零。边界按 §2.2 向可行侧投影，`xTargetMin=xTargetLow`；上界为内部求解量，本次不新增 JSON/CSV 字段。
...
```text
Ω > 0 -> xTargetMinCost = xTargetLow
Ω < 0 -> xTargetMinCost = xTargetHigh
Ω = 0 -> [xTargetLow, xTargetHigh]全部并列，代表比例取xTargetLow
```

候选燃料按同等物理能源更便宜时，增加混兑比例可能继续降低当前模型成本；若它使 GHGI 恶化，仍必须止于达标上界。候选级目标建议在输出前再次验证方案可行且合规余额非负，否则返回 `UNAVAILABLE / TARGET_NOT_SATISFIED`。
...
S_report = 去重后的集合：
  ...
  用户指定比例
  xTargetMin
  xTargetMinCost（若达标上界形成新的必要报告点）
  有效约束边界（xCap、预算/供应量边界、约束下最大改善比例）
```

集合按未舍入比例去重。B100仅在允许纯用时作为参考，受约束不可行时不进入“约束下”排序。`xTargetMinCost`通常复用`xTargetMin`或有效约束边界；B0已达标且便宜候选使GHGI恶化时，它可以是新的达标上界，必须加入报告集合。新增最优点同样遵守B100纯用限制；不得用 `1-epsilon` 伪造替代比例。
...
切换点和区间赢家使用精确有理数比较，仅数学相同的交点去重，不以固定 `1e-30` 阈值合并。航次/案例编排从共享能源、组分、单位成本和GHGI规则生成精确成本线，避免各方案Decimal输出尾差制造本来不存在的微小区间。单独传入方案结果进行比较时，以调用者提供的成本和改善值作为精确输入。
